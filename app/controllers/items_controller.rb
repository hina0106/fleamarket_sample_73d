class ItemsController < ApplicationController
  ...
  def index
    @items_category = Item.where("buyer_id IS NULL AND category_id < 200").order(created_at: "DESC")
    @items_brand = Item.where("buyer_id IS NULL AND brand_id = 1").order(created_at: "DESC")
  end
